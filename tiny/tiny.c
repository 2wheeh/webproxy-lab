...
void read_requesthdrs(rio_t *rp) /* 친절해서 출력해줌 헤더 읽어주는 남자 */
{
  char buf[MAXLINE];
...
{ 
  ...
  if(!strstr(uri, "cgi-bin")){ /* Static content : cig-bin이라는 substring이 없다면 (null을 반환) */
    strcpy(cgiargs, "");  /* clear cgiargs which is for dynamic content */
    strcpy(filename, "."); /* filename에 . 넣어줌 */
    strcat(filename, uri); /* .뒤에 uri 붙여줌 */
    if (uri[strlen(uri)-1] == '/') /* uri(filename)이 /로 끝나면 filename에 home.html 추가 */
      strcat(filename, "home.html");
    return 1;
  }
  else { /* cgi-bin이 있으면 */
    ptr = index(uri, '?');  // '?' 가 시작하는 주소를 ptr에 저장
    if(ptr) {
      strcpy(cgiargs, ptr+1); /* ?뒤의 내용을 cgiargs에 넣어줌 */
      *ptr = '\0';  /* ?를 개행문자 '\0' 로 바꿔줌 그 뒤를 uri에서 버린다는 것 */ 
    }
    else /* uri에 ?가 없으면 */
      strcpy(cgiargs, "");  /* cgiargs에 넣어줄게 없으니까 clear cgiargs */
    
    strcpy(filename, ".");  /* filename에 . 넣어줌 */
    strcat(filename, uri); /* .뒤에 uri 붙여줌 (?뒤의 내용은 cgiargs로 옮겨놧음) */
    return 0;
  }
}
